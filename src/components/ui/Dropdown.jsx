...
const Dropdown = ({ value, options, onChange, label, placeholder, className = '', buttonClassName = '', menuClassName = '' }) => {
        ...
        <AnimatePresence>
          {isOpen && (
              <motion.div
                ...
                className={`absolute right-0 origin-top-right z-[100] top-full mt-2 min-w-[150px] bg-white dark:bg-[#141417] backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-xl shadow-xl dark:shadow-[0_20px_50px_rgba(0,0,0,0.5)] overflow-y-auto overflow-x-hidden max-h-[250px] custom-scrollbar ${menuClassName}`}
              >
                {options.map((opt) => (
                  <button
                    key={opt.value}
                    ...
                    className={`w-full text-left px-4 py-2.5 text-sm transition-all flex items-center justify-between
                      ${value === opt.value ? 'bg-primary/20 text-primary font-bold' : 'text-text hover:bg-slate-100 dark:hover:bg-white/10'}
                    `}
                  >
                    <span>{opt.label}</span>
